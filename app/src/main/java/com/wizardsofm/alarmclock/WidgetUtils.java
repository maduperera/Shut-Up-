...
public final class WidgetUtils {
    ...
    public static float getScaleRatio(Context context, Bundle options, int id, int cityCount) {
        ...
        if (options != null) {
            ...
            float ratio = (density * minWidth) / res.getDimension(com.wizardsofm.deskclock.R.dimen.min_digital_widget_width);
    ...
    private static float getHeightScaleRatio(Context context, Bundle options, int id) {
        ...
        if (options != null) {
            ...
            final Resources res = context.getResources();
            float density = res.getDisplayMetrics().density;
            float ratio = density * minHeight / res.getDimension(com.wizardsofm.deskclock.R.dimen.min_digital_widget_height);
            if (Utils.isPortrait(context)) {
                return ratio * 1.75f;
            }
            return ratio;

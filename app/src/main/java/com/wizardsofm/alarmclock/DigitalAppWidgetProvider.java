...
package com.wizardsofm.alarmclock;
...
import static com.wizardsofm.deskclock.alarms.AlarmStateManager.ACTION_ALARM_CHANGED;
import static com.wizardsofm.deskclock.data.DataModel.ACTION_WORLD_CITIES_CHANGED;
...
public class DigitalAppWidgetProvider extends AppWidgetProvider {
    ...
    public void onReceive(@NonNull Context context, @NonNull Intent intent) {
        ...
        dm.updateWidgetCount(getClass(), widgetIds.length, com.wizardsofm.deskclock.R.string.category_digital_widget);
    ...
    private static void relayoutWidget(Context context, AppWidgetManager wm, int widgetId,
        ...
        wm.notifyAppWidgetViewDataChanged(widgetId, com.wizardsofm.deskclock.R.id.world_city_list);
    }
    ...
    private static RemoteViews relayoutWidget(Context context, AppWidgetManager wm, int widgetId,
        ...
        final RemoteViews rv = new RemoteViews(packageName, com.wizardsofm.deskclock.R.layout.digital_widget);

        // Tapping on the widget opens the app (if not on the lock screen).
        if (Utils.isWidgetClickable(wm, widgetId)) {
            final Intent openApp = new Intent(context, DeskClock.class);
            final PendingIntent pi = PendingIntent.getActivity(context, 0, openApp, 0);
            rv.setOnClickPendingIntent(com.wizardsofm.deskclock.R.id.digital_widget, pi);
        }

        // Configure child views of the remote view.
        final CharSequence dateFormat = getDateFormat(context);
        rv.setCharSequence(com.wizardsofm.deskclock.R.id.date, "setFormat12Hour", dateFormat);
        rv.setCharSequence(com.wizardsofm.deskclock.R.id.date, "setFormat24Hour", dateFormat);

        final String nextAlarmTime = Utils.getNextAlarm(context);
        if (TextUtils.isEmpty(nextAlarmTime)) {
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.nextAlarm, GONE);
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.nextAlarmIcon, GONE);
        } else  {
            rv.setTextViewText(com.wizardsofm.deskclock.R.id.nextAlarm, nextAlarmTime);
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.nextAlarm, VISIBLE);
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.nextAlarmIcon, VISIBLE);
        }
        ...
        final int largestClockFontSizePx =
                resources.getDimensionPixelSize(com.wizardsofm.deskclock.R.dimen.widget_max_clock_font_size);
        ...
        rv.setImageViewBitmap(com.wizardsofm.deskclock.R.id.nextAlarmIcon, sizes.mIconBitmap);
        rv.setTextViewTextSize(com.wizardsofm.deskclock.R.id.date, COMPLEX_UNIT_PX, sizes.mFontSizePx);
        rv.setTextViewTextSize(com.wizardsofm.deskclock.R.id.nextAlarm, COMPLEX_UNIT_PX, sizes.mFontSizePx);
        rv.setTextViewTextSize(com.wizardsofm.deskclock.R.id.clock, COMPLEX_UNIT_PX, sizes.mClockFontSizePx);

        final int smallestWorldCityListSizePx =
                resources.getDimensionPixelSize(com.wizardsofm.deskclock.R.dimen.widget_min_world_city_list_size);
        if (sizes.getListHeight() <= smallestWorldCityListSizePx) {
            // Insufficient space; hide the world city list.
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.world_city_list, GONE);
        } else {
            ...
            rv.setRemoteAdapter(com.wizardsofm.deskclock.R.id.world_city_list, intent);
            rv.setViewVisibility(com.wizardsofm.deskclock.R.id.world_city_list, VISIBLE);
            ...
            if (Utils.isWidgetClickable(wm, widgetId)) {
                ...
                rv.setPendingIntentTemplate(com.wizardsofm.deskclock.R.id.world_city_list, pi);
            }
        }
    ...
    private static Sizes optimizeSizes(Context context, Sizes template, String nextAlarmTime) {
        ...
        final View sizer = inflater.inflate(com.wizardsofm.deskclock.R.layout.digital_widget_sizer, null /* root */);

        // Configure the date to display the current date string.
        final CharSequence dateFormat = getDateFormat(context);
        final TextClock date = (TextClock) sizer.findViewById(com.wizardsofm.deskclock.R.id.date);
        date.setFormat12Hour(dateFormat);
        date.setFormat24Hour(dateFormat);

        // Configure the next alarm views to display the next alarm time or be gone.
        final TextView nextAlarmIcon = (TextView) sizer.findViewById(com.wizardsofm.deskclock.R.id.nextAlarmIcon);
        final TextView nextAlarm = (TextView) sizer.findViewById(com.wizardsofm.deskclock.R.id.nextAlarm);
        if (TextUtils.isEmpty(nextAlarmTime)) {
    ...
    private static Sizes measure(Sizes template, int clockFontSize, View sizer) {
        ...
        final TextClock date = (TextClock) sizer.findViewById(com.wizardsofm.deskclock.R.id.date);
        final TextClock clock = (TextClock) sizer.findViewById(com.wizardsofm.deskclock.R.id.clock);
        final TextView nextAlarm = (TextView) sizer.findViewById(com.wizardsofm.deskclock.R.id.nextAlarm);
        final TextView nextAlarmIcon = (TextView) sizer.findViewById(com.wizardsofm.deskclock.R.id.nextAlarmIcon);
    ...
    private static String getDateFormat(Context context) {
        ...
        final String skeleton = context.getString(com.wizardsofm.deskclock.R.string.abbrev_wday_month_day_no_year);

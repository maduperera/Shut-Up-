...
public class HandleDeskClockApiCalls extends Activity {
    ...
    private void handleStopwatchIntent(Intent intent) {
        ...
        final int eventLabel = intent.getIntExtra(EXTRA_EVENT_LABEL, com.wizardsofm.deskclock.R.string.label_intent);

        if (ACTION_SHOW_STOPWATCH.equals(action)) {
            Events.sendStopwatchEvent(com.wizardsofm.deskclock.R.string.action_show, eventLabel);
        } else {
            final String reason;
            boolean fail = false;
            switch (action) {
                case ACTION_START_STOPWATCH: {
                    ...
                    Events.sendStopwatchEvent(com.wizardsofm.deskclock.R.string.action_start, eventLabel);
                    reason = getString(com.wizardsofm.deskclock.R.string.stopwatch_started);
                    break;
                }
                case ACTION_PAUSE_STOPWATCH: {
                    DataModel.getDataModel().pauseStopwatch();
                    Events.sendStopwatchEvent(com.wizardsofm.deskclock.R.string.action_pause, eventLabel);
                    reason = getString(com.wizardsofm.deskclock.R.string.stopwatch_paused);
                    break;
                }
                case ACTION_RESET_STOPWATCH: {
                    DataModel.getDataModel().resetStopwatch();
                    Events.sendStopwatchEvent(com.wizardsofm.deskclock.R.string.action_reset, eventLabel);
                    reason = getString(com.wizardsofm.deskclock.R.string.stopwatch_reset);
                    break;
                }
                case ACTION_LAP_STOPWATCH: {
                    if (!DataModel.getDataModel().getStopwatch().isRunning()) {
                        fail = true;
                        reason = getString(com.wizardsofm.deskclock.R.string.stopwatch_isnt_running);
                    } else {
                        DataModel.getDataModel().addLap();
                        Events.sendStopwatchEvent(com.wizardsofm.deskclock.R.string.action_lap, eventLabel);
                        reason = getString(com.wizardsofm.deskclock.R.string.stopwatch_lapped);
                    }
                    break;
                }
                default:
    ...
    private void handleTimerIntent(Intent intent) {
        ...
        if (ACTION_SHOW_TIMERS.equals(action)) {
            Events.sendTimerEvent(com.wizardsofm.deskclock.R.string.action_show, eventLabel);
        } else {
            String reason = null;
            if (timerId == -1) {
                ...
                if (timers.isEmpty()) {
                    ...
                    reason = getString(com.wizardsofm.deskclock.R.string.no_timers_exist);
                } else if (timers.size() > 1) {
                    // Many timers exist so the control command is ambiguous.
                    reason = getString(com.wizardsofm.deskclock.R.string.too_many_timers_exist);
                } else {
                    timer = timers.get(0);
                }
            } else {
                ...
                if (timer == null) {
                    reason = getString(com.wizardsofm.deskclock.R.string.timer_does_not_exist);
                }
            }
            ...
            if (timer == null) {
                ...
                switch (action) {
                    case ACTION_RESET_TIMER: {
                        ...
                        if (timer == null) {
                            timerId = -1;
                            reason = getString(com.wizardsofm.deskclock.R.string.timer_deleted);
                        } else {
                            reason = getString(com.wizardsofm.deskclock.R.string.timer_was_reset);
                        }
                        break;
                    }
                    case ACTION_START_TIMER: {
                        DataModel.getDataModel().startTimer(timer);
                        Events.sendTimerEvent(com.wizardsofm.deskclock.R.string.action_start, eventLabel);
                        reason = getString(com.wizardsofm.deskclock.R.string.timer_started);
                        break;
                    }
                    case ACTION_PAUSE_TIMER: {
                        DataModel.getDataModel().pauseTimer(timer);
                        Events.sendTimerEvent(com.wizardsofm.deskclock.R.string.action_pause, eventLabel);
                        reason = getString(com.wizardsofm.deskclock.R.string.timer_paused);
                        break;
                    }
                    default:
    ...
    private void handleClockIntent(Intent intent) {
        ...
        if (ACTION_SHOW_CLOCK.equals(action)) {
            Events.sendClockEvent(com.wizardsofm.deskclock.R.string.action_show, eventLabel);
        } else {
            switch (action) {
                case ACTION_ADD_CLOCK:
                    Events.sendClockEvent(com.wizardsofm.deskclock.R.string.action_add, eventLabel);
                    break;
                case ACTION_DELETE_CLOCK:
                    Events.sendClockEvent(com.wizardsofm.deskclock.R.string.action_delete, eventLabel);
                    break;
            }
            ...
            if (cityName == null) {
                reason = getString(com.wizardsofm.deskclock.R.string.no_city_selected);
                LogUtils.i(reason);
                Voice.notifySuccess(this, reason);
            ...
            if (city == null) {
                reason = getString(com.wizardsofm.deskclock.R.string.the_city_you_specified_is_not_available);
                LogUtils.i(reason);
                Voice.notifyFailure(this, reason);
                startActivity(new Intent(this, CitySelectionActivity.class)
                        .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK));
            ...
            switch (action) {
                case ACTION_ADD_CLOCK: {
                    ...
                    if (!selectedCities.add(city)) {
                        fail = true;
                        reason = getString(com.wizardsofm.deskclock.R.string.the_city_already_added);
                        break;
                    }

                    // Otherwise report the success.
                    DataModel.getDataModel().setSelectedCities(selectedCities);
                    reason = getString(com.wizardsofm.deskclock.R.string.city_added, city.getName());
                    break;
                }
                case ACTION_DELETE_CLOCK: {
                    ...
                    // Otherwise report the success.
                    DataModel.getDataModel().setSelectedCities(selectedCities);
                    reason = getString(com.wizardsofm.deskclock.R.string.city_deleted, city.getName());
                    break;
                }
                default:

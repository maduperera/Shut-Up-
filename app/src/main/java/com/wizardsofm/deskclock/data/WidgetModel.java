...
final class WidgetModel {
    ...
    void updateWidgetCount(Class widgetClass, int count, @StringRes int eventCategoryId) {
        ...
        for (; delta > 0; delta--) {
            Events.sendEvent(eventCategoryId, com.wizardsofm.deskclock.R.string.action_create, 0);
        }
        for (; delta < 0; delta++) {
            Events.sendEvent(eventCategoryId, com.wizardsofm.deskclock.R.string.action_delete, 0);
        }
    }
}
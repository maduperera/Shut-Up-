...
public class CrescendoLengthDialogPreference extends DialogPreference {
    ...
    public void updateSummary() {
        ...
        if (crescendoSeconds == 0) {
            setSummary(getContext().getString(com.wizardsofm.deskclock.R.string.no_crescendo_duration));
        } else {
            final String length = UiDataModel.getUiDataModel().getFormattedNumber(crescendoSeconds);
            setSummary(getContext().getString(com.wizardsofm.deskclock.R.string.crescendo_duration, length));
        }
    }
}
